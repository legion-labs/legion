...
impl Drop for VulkanDescriptorSetLayoutInner {
    fn drop(&mut self) {
        unsafe {
            self.device_context.device().destroy_descriptor_set_layout(
                self.vk_layout, 
                None
            )
        }
...
impl VulkanDescriptorSetLayout {
    ...
    pub(crate) fn find_descriptor_index_by_name(&self, name: &str ) -> Option<u32> {
        self.inner.descriptors.iter().position( |descriptor| &descriptor.name == name ).map( |opt| opt as u32 )
    }

    pub(crate) fn descriptor(&self, index: u32) -> GfxResult<&VulkanDescriptor> {
        self.inner.descriptors.get(index as usize).ok_or( GfxError::from( "Invalid descriptor index" ) )
    }
    ...
    pub(crate) fn new(
        ...
        for descriptor_def in &descriptor_set_layout_def.descriptor_defs {
            ...
            super::util::resource_type_to_descriptor_type(descriptor_def.resource_type).unwrap();
            ...
            stage_flags(vk::ShaderStageFlags::ALL).build();            
