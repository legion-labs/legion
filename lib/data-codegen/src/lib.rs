...
/// # Errors
pub fn generate_for_directory(directory: &std::path::Path) -> anyhow::Result<()> {
    ...
    [GenerationType::OfflineFormat, GenerationType::RuntimeFormat]
        .into_iter()
        .try_for_each(|gen_type| -> anyhow::Result<()> {
...
pub fn generate_data_container_code(
...
) -> anyhow::Result<Vec<DataContainerMetaInfo>> {
    ...
    writeln!(gen_file, "#![allow(dead_code)]")?;
    writeln!(gen_file, "#![allow(clippy::needless_update)]\n")?;
    ...
    // Add uses inside the mod
    uses_token
        .iter()
        .try_for_each(|use_token| gen_file.write_all(use_token.to_string().as_bytes()))?;
...
macro_rules! data_container_gen {
    ( $( $x:expr ),* ) => {
        let mut data_path = env!("CARGO_MANIFEST_DIR").to_lowercase();
        $(
            data_path.push_str($x);
            lgn_data_codegen::generate_data_container_code(std::path::Path::new(&data_path)
