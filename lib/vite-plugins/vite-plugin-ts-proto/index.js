 ...
 */
function resolveTsProtoPath(binName, baseDir) {
...
function resolveTsProtoBinName() {
...
async function generateProtoForModule(module, { baseDir, protocPath, tsProtoPath, quiet, }) {
    const cwd = path.resolve(baseDir, "node_modules", module.name);
    const globMatches = await new Promise((resolve, reject) => glob(module.glob, { cwd }, (error, matches) => error ? reject(error) : resolve(matches)));
    if (!globMatches) {
        console.warn(`No proto files matching glob ${module.glob} for ${module.name}`);
        return;
    }
 ...
 */
export default function vitePluginProtoPack({ modules, quiet = false, }) {
    return {
        name: "vite-plugin-ts-proto",
        async buildStart() {
            const baseDir = process.cwd();
            const protocPath = await new Promise((resolve, reject) => which("protoc", (error, path) => error ? reject(error) : resolve(path)));
            if (!protocPath) {
                // eslint-disable-next-line no-console
            ...
            await Promise.all(modules.map((module) => generateProtoForModule(module, {
                baseDir,
                protocPath,
                tsProtoPath,
                quiet,
