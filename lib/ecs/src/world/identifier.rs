...
// Note that this *is* used by external crates as well as for internal safety
// checks
...
impl WorldId {
    /// Create a new, unique [`WorldId`]. Returns [`None`] if the supply of
    /// unique [`WorldId`]s has been exhausted
    ///
    /// Please note that the [`WorldId`]s created from this method are unique
    /// across time - if a given [`WorldId`] is [`Drop`]ped its value still
    /// cannot be reused
