...
impl AwsCognitoClientAuthenticator {
    ...
    fn get_authorization_url(&self) -> String {
        ...
        url.query_pairs_mut()
            ...
            .append_pair("redirect_uri", self.get_redirect_uri().as_str());
    ...
    pub fn get_authorization_code_interactive(&self) {
        ...
        window
            .location()
            .set_href(authorization_url.as_str())
            .unwrap();
    }

    pub async fn get_token_set_from_authorization_code(
        &self,
        code: &str,
        ...
        body.append("client_id", self.client_id.as_str());
        body.append("code", code);
        body.append("redirect_uri", self.get_redirect_uri().as_str());
        ...
        let request =
            Request::new_with_str_and_init(access_token_url.as_str(), &request_init).unwrap();
    ...
    pub async fn get_user_info(&self, access_token: &str) -> anyhow::Result<UserInfo> {
        ...
        let request =
            Request::new_with_str_and_init(user_info_url.as_str(), &request_init).unwrap();
...
impl TokenCache {
    ...
    pub fn get_authorization_code_interactive(&self) -> anyhow::Result<()> {
        if Self::client_token_set_is_valid() {
            Err(UserAlreadyLoggedInError).map_err(anyhow::Error::msg)
        } else {
            self.authenticator.get_authorization_code_interactive();
    ...
    pub async fn get_token_set_from_authorization_code(
        ...
        if Self::client_token_set_is_valid() {
            ...
            self.authenticator
                .get_token_set_from_authorization_code(code)
                .await
        }
    }
