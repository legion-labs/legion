...
static COMPILER_INFO: CompilerDescriptor = CompilerDescriptor {
    ...
    data_version: "1",
    transform: &(
        offline_data::MATERIAL_TYPE_ID.content(),
        runtime_data::MATERIAL_TYPE_ID.content(),
    ),
...
fn compile(context: CompilerContext<'_>) -> Result<CompilationOutput, CompilerError> {
    ...
    let resource = resource.get::<offline_data::Material>(&resources).unwrap();

    let asset = runtime_data::Material::from_offline(resource);
