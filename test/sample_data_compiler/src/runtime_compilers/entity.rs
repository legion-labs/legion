...
static COMPILER_INFO: CompilerDescriptor = CompilerDescriptor {
    ...
    data_version: "1",
    transform: &(
        offline_data::ENTITY_TYPE_ID.content(),
        runtime_data::ENTITY_TYPE_ID.content(),
    ),
...
fn compile(context: CompilerContext<'_>) -> Result<CompilationOutput, CompilerError> {
    ...
    let resource = resource.get::<offline_data::Entity>(&resources).unwrap();

    let asset = runtime_data::Entity::from_offline(resource);
