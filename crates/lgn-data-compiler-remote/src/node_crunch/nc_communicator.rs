...
impl NCCommunicator {
    ...
    pub(crate) fn nc_encode_data<S: Serialize>(&mut self, data: &S) -> Result<Vec<u8>, NCError> {
        let mut data_out = serialize(data).map_err(NCError::Serialize)?;
    ...
    pub(crate) fn nc_send_data<S: Serialize, A: ToSocketAddrs>(
        &mut self,
        data: &S,
        socket_addr: &A,
    ) -> Result<(), NCError> {
    ...
    ///
    /// On failure it returns a [`NCError`].
    pub(crate) fn nc_send_data2<S: Serialize, W: Write>(
        &mut self,
        data: &S,
        tcp_stream: &mut W,
    ) -> Result<(), NCError> {
    ...
    ///
    /// On failure it returns a [`NCError`].
    pub(crate) fn nc_receive_data<D: DeserializeOwned, R: Read>(
        &self,
        tcp_stream: &mut R,
    ) -> Result<D, NCError> {
    ...
    ///
    /// On failure it returns a [`NCError`].
    pub(crate) fn nc_send_receive_data<S: Serialize, D: DeserializeOwned, A: ToSocketAddrs>(
        &mut self,
        data: &S,
        socket_addr: &A,
    ) -> Result<D, NCError> {
...
mod tests {
    ...
    fn test_receive_data() {
        ...
        let data3: (String, u32, bool) = nc_communicator
            .nc_receive_data(&mut buffer.as_slice())
            .unwrap();
    ...
    fn test_send_and_receive() {
        ...
        let data1: (String, u32, bool) =
            ("Test send and then receive data!".to_string(), 550055, true);

        let mut buffer: Vec<u8> = Vec::new();

        nc_communicator.nc_send_data2(&data1, &mut buffer).unwrap();

        let data2: (String, u32, bool) = nc_communicator
            .nc_receive_data(&mut buffer.as_slice())
            .unwrap();
