...
pub(crate) enum NCNodeMessage<NodeTypeT, ProcessedDataT, CustomMessageT> {
...
/// Tells what the node should do after processing one payload from the server.
#[derive(Debug, Serialize, Deserialize)]
pub enum NCNodeStatus<ProcessedDataT> {
    /// The node can carry on operations.
    Ready(ProcessedDataT),
    /// The node must disconnect from the server.
    Exiting,
}

/// This trait has to be implemented for the code that runs on all the nodes.
pub trait NCNode {
    type NodeTypeT: Serialize + DeserializeOwned;
    ...
    /// Upon node registration, inform the server of which kind of node we are.
    /// # Errors
...
impl<'a> NCNodeStarter {
    ...
    fn start_heartbeat_thread(
        mut node_heartbeat: NodeHeartbeat,
        rx: Receiver<bool>,
    ) -> JoinHandle<()> {
    ...
    fn start_main_loop<T: NCNode>(node_process: &mut NodeProcess<'_, T>) {
        ...
        loop {
            ...
            let result = node_process.get_and_process_data();
            match result {
                Err(e) => {
                    error!(
...
struct NodeProcess<'a, T> {
    /// IP address and port of the server.
    server_addr: Arc<Mutex<SocketAddr>>,
    /// The super defined data structure that implements the NCNode trait.
    nc_node: &'a mut T,
    /// The node id for this node,
    node_id: NodeID,
...
impl<'a, T: NCNode> NodeProcess<'a, T> {
    ...
    fn get_initial_data(&mut self) -> Result<(), NCError> {
        ...
        let node_type = self.nc_node.get_node_type()?;

        let initial_data: NCServerMessage<T::InitialDataT, T::NewDataT, T::CustomMessageT> =
            self.send_register_message(node_type)?;
        ...
        match initial_data {
            ...
            _msg => {
                error!(
                    "Error in get_initial_data(), NCServerMessage mismatch, expected: InitialData"
                );
    ...
    fn send_register_message(
        ...
        let message: NCNodeMessage<T::NodeTypeT, T::ProcessedDataT, T::CustomMessageT> =
            NCNodeMessage::Register(node_type);
        let server_addr = *self.server_addr.lock()?;

        self.nc_communicator
            .nc_send_receive_data(&message, &server_addr)
    }
    ...
    fn get_and_process_data(&mut self) -> Result<bool, NCError> {
        debug!("NodeProcess::get_and_process_data()");

        let new_data: NCServerMessage<T::InitialDataT, T::NewDataT, T::CustomMessageT> =
    ...
    fn send_needs_data_message(
        ...
        let message: NCNodeMessage<T::NodeTypeT, T::ProcessedDataT, T::CustomMessageT> =
            NCNodeMessage::NeedsData(self.node_id);
        let server_addr = *self.server_addr.lock()?;
    ...
    fn process_data_and_send_has_data_message(
        &mut self,
        data: &T::NewDataT,
    ) -> Result<bool, NCError> {
        debug!("NodeProcess::process_data_and_send_has_data_message()");
        if let NCNodeStatus::Ready(data) = self.nc_node.process_data_from_server(data)? {
            let message: NCNodeMessage<T::NodeTypeT, T::ProcessedDataT, T::CustomMessageT> =
                NCNodeMessage::HasData(self.node_id, data);
            let server_addr = *self.server_addr.lock()?;

            self.nc_communicator.nc_send_data(&message, &server_addr)?;
            return Ok(true); // continue treating messages
        }
        Ok(false) // exiting
    }
    ...
    fn send_node_migrated(&mut self) -> Result<(), NCError> {
        ...
        let message: NCNodeMessage<T::NodeTypeT, T::ProcessedDataT, T::CustomMessageT> =
            NCNodeMessage::NodeMigrated(self.node_id);
        let server_addr = *self.server_addr.lock()?;
...
mod tests {
    ...
    impl NCNode for TestNode {
        ...
        fn process_data_from_server(&mut self, _data: &()) -> Result<NCNodeStatus<()>, NCError> {
            Ok(NCNodeStatus::Ready(()))
        }
    }
    ...
    fn test_np_dec_and_check_counter() {
        let mut nc_node = TestNode {};
        let config = NCConfiguration::default();
        let server_addr = server_addr_for_test();
        let mut np = NodeProcess::new(server_addr, &mut nc_node, &config);
