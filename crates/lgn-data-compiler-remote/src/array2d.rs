...
impl<T: Clone + Copy> Array2DChunk<T> {
    ...
    pub fn get_chunk_property(&self, chunk_id: u64) -> (u64, u64, u64, u64) {
        ...
        let width = if (cx == (self.chunks_x - 1)) && (self.rest_width > 0) {
            self.rest_width
        } else {
            self.chunk_width
        };
        let height = if (cy == (self.chunks_y - 1)) && (self.rest_height > 0) {
            self.rest_height
        } else {
            self.chunk_height
        };
    ...
    pub fn set_chunk(&mut self, chunk_id: u64, source: &Array2D<T>) -> Result<(), NCError> {
        ...
        if (width == source.width) && (height == source.height) {
            ...
            Err(NCError::Array2DDimensionMismatch(
                (width, height),
...
impl<T> Chunk<T> {
    ...
    pub fn is_processing(&self, node_id: NodeID) -> bool {
        self.status == ChunkStatus::Processing && self.node_id == node_id
    }
