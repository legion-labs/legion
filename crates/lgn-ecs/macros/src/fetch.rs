...
pub fn derive_world_query_impl(ast: DeriveInput) -> TokenStream {
    ...
    assert!(
        !fetch_struct_attributes.is_filter || !fetch_struct_attributes.is_mutable,
    ...
    if fetch_struct_attributes.is_filter {
        assert!(!has_world_lifetime, "Expected a struct without a lifetime");
    } else {
        assert!(has_world_lifetime, "Expected a struct with a lifetime");
    }
