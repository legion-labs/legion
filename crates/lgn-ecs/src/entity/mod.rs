...
//!
//! In Legion ECS, there is no monolithic data structure for an entity. Instead,
//! the [`Entity`] `struct` is just a *generational index* (a combination of an
//! ID and a generation). Then, the `Entity` maps to the specific
//! [`Component`s](crate::component::Component). This way, entities can have
//! meaningful data attached to it. This is a fundamental design choice that has
//! been taken to enhance performance and usability.
//!
//! # Usage
//!
...
/// [`Commands::spawn`](crate::system::Commands::spawn). Can be stored to refer
/// to an entity in the future.
///
...
impl Entity {
    ...
    ///
    /// Only useful for identifying entities within the same instance of an
    /// application. Do not use for serialization between runs.
    ///
    ...
    /// Reconstruct an `Entity` previously destructured with
    /// [`Entity::to_bits`].
    ///
    /// Only useful when applied to results from `to_bits` in the same instance
    /// of an application.
    ...
    ///
    /// No two simultaneously-live entities share the same ID, but dead
    /// entities' IDs may collide with both live and dead entities. Useful
    /// for compactly representing entities within a specific snapshot of
    /// the world, such as when serializing.
    #[inline]
    pub fn id(self) -> u32 {
        self.id
    }

    /// Returns the generation of this Entity's id. The generation is
    /// incremented each time an entity with a given id is despawned. This
    /// serves as a "count" of the number of times a given id has been
    /// reused (id, generation) pairs uniquely identify a given Entity.
    #[inline]
...
pub struct Entities {
    ...
    ///
    /// - The `freelist` IDs, previously freed by `free()`. These IDs are
    ///   available to any of `alloc()`, `reserve_entity()` or
    ///   `reserve_entities()`. Allocation will always prefer these over brand
    ///   new IDs.
    ///
    /// - The `reserved` list of IDs that were once in the freelist, but got
    ///   reserved by `reserve_entities` or `reserve_entity()`. They are now
    ///   waiting for `flush()` to make them fully allocated.
    ///
    /// - The count of new IDs that do not yet exist in `self.meta()`, but which
    ///   we have handed out and reserved. `flush()` will allocate room for them
    ///   in `self.meta()`.
    ///
    ...
    /// items from the freelist into the reserved list by sliding over the
    /// boundary.
    ///
    /// Once the freelist runs out, `free_cursor` starts going negative.
    /// The more negative it is, the more IDs have been reserved starting
    /// exactly at the end of `meta.len()`.
    ///
    /// This formulation allows us to reserve any number of IDs first from the
    /// freelist and then from the new IDs, using only a single atomic
    /// subtract.
    ///
...
impl Entities {
    ...
    pub fn reserve_entities(&self, count: u32) -> ReserveEntitiesIterator<'_> {
        ...
        let (new_id_start, new_id_end) = if range_start >= 0 {
            ...
            // Then we negate these values to indicate how far beyond the end of
            // `meta.end()` to go, yielding `meta.len()+0 .. meta.len()+3`.
    ...
    ///
    /// Equivalent to `self.reserve_entities(1).next().unwrap()`, but more
    /// efficient.
    ...
    /// Get the [`Entity`] with a given id, if it exists in this [`Entities`]
    /// collection Returns `None` if this [`Entity`] is outside of the range
    /// of currently reserved Entities
    ///
    /// Note: This method may return [`Entities`](Entity) which are currently
    /// free Note that [`contains`](Entities::contains) will correctly
    /// return false for freed entities, since it checks the generation
    pub fn resolve_from_id(&self, id: u32) -> Option<Entity> {
        let idu = id as usize;
        if let Some(&EntityMeta { generation, .. }) = self.meta.get(idu) {
            Some(Entity { generation, id })
        } else {
            // `id` is outside of the meta list - check whether it is reserved but not yet
            // flushed.
    ...
    /// or `reserve_entities`, then initializes each one using the supplied
    /// function.
    ///
    /// # Safety
    ...
    // Flushes all reserved entities to an "invalid" state. Attempting to retrieve
    // them will return None unless they are later populated with a valid
    // archetype.
