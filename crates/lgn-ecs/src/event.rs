...
impl<'w, 's, E: Event> EventReader<'w, 's, E> {
    ...
    ///     }
    /// }
    /// # lgn_ecs::system::assert_is_system(play_collision_sound);
    /// ```
...
mod tests {
    ...
    fn test_event_reader_clear() {
        ...
        let mut reader =
            IntoSystem::into_system(|events: EventReader<'_, '_, TestEvent>| -> bool {
                if !events.is_empty() {
                    events.clear();
                    false
                } else {
    ...
    fn ensure_reader_readonly() {
        fn read_for<E: Event>() {
            ...
            let mut state = SystemState::<EventReader<'_, '_, E>>::new(&mut world);
