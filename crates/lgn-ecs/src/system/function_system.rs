...
///
/// use lgn_ecs::prelude::*;
/// use lgn_ecs::system::{SystemParam, SystemParamItem};
///
...
pub trait SystemParamFunction<In, Out, Param: SystemParam, Marker>: Send + Sync + 'static {
    fn run(&mut self, input: In, param_value: SystemParamItem<'_, '_, Param>) -> Out;
}
...
macro_rules! impl_system_function {
    ($($param: ident),*) => {
        ...
        for <'a> &'a mut Func:
                FnMut($($param),*) -> Out +
                FnMut($(SystemParamItem<'_, '_, $param>),*) -> Out, Out: 'static
        {
            #[inline]
        ...
        {
            #[inline]
            fn run(&mut self, input: Input, param_value: SystemParamItem<'_, '_,  ($($param,)*)>) -> Out {
                #[allow(clippy::too_many_arguments)]
