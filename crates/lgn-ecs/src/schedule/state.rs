...
mod test {
    ...
    fn restart_state_tests() {
        ...
        stage
            .add_system_set(
                State::on_enter_set(LoadState::Load).with_system(
                    |mut r: ResMut<'_, Vec<LoadStatus>>| r.push(LoadStatus::EnterLoad),
                ),
            )
            .add_system_set(
                State::on_exit_set(LoadState::Load)
                    .with_system(|mut r: ResMut<'_, Vec<LoadStatus>>| r.push(LoadStatus::ExitLoad)),
            )
            .add_system_set(
                State::on_enter_set(LoadState::Finish).with_system(
                    |mut r: ResMut<'_, Vec<LoadStatus>>| r.push(LoadStatus::EnterFinish),
                ),
            );
