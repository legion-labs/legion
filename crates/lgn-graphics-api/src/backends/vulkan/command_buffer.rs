    ...
    pub(crate) fn backend_cmd_begin_render_pass(
        ...
        let (depth_attachment, stencil_attachment) =
            if let Some(depth_target) = depth_target.as_ref() {
                let format = depth_target.texture_view.texture().definition().format;

                let depth_attachment = if format.has_depth() {
                    Some(
                ...
                let stencil_attachment = if format.has_stencil() {
                    Some(
                        ash::vk::RenderingAttachmentInfo::builder()
                            .image_view(depth_target.texture_view.vk_image_view())
                            .image_layout(ash::vk::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
        ...
        let mut render_info = ash::vk::RenderingInfo::builder()
        ...
        if let Some(stencil_attachment) = &stencil_attachment {
            render_info = render_info.stencil_attachment(stencil_attachment);
        }
