...
impl MeshRenderer {
    ...
    pub(crate) fn gen_occlusion_and_cull(
        ...
        let gpu_instance_view =
            gpu_instance_allocation.to_buffer_view(BufferViewDef::as_structured_buffer_typed::<
                GpuInstanceData,
            >(
                self.gpu_instance_data.len() as u64, true
            ));
...
fn create_or_replace_buffer(
    ...
    if buffer.is_none() {
        let new_buffer = device_context.create_buffer(BufferDef {
            size: required_size,
            usage_flags,
