...
use crate::{
    AssetRegistry, AssetRegistryError, AssetRegistryReader, HandleUntyped, LoadRequest,
    ReferenceUntyped, Resource, ResourceTypeAndId,
};

/// Implement a default `ResourceInstaller` using `from_reader` interface
#[macro_export]
macro_rules! implement_default_resource_installer {
    ($loader: ident, $type_id: ident) => {
        #[derive(Default)]
        struct $loader {}
        #[async_trait::async_trait]
        impl crate::ResourceInstaller for $loader {
            async fn install_from_stream(
                ...
                let handle = request
                    .asset_registry
                    .set_resource(resource_id, Box::new($type_id::from_reader(reader).await?))?;
                Ok(handle)
            }
        }
    };
