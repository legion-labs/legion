...
use crate::{plugin::get_script_payload, runtime::ScriptComponent, ScriptType, ScriptingStage};
...
fn compile(
    ...
    for (entity, script) in rhai_scripts {
        ...
        let script_payload = std::str::from_utf8(script_payload).unwrap();
        println!("{}", &script_payload);

        let ast = rhai_eng.compile(script_payload).unwrap();
