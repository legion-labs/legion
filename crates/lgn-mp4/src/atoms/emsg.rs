...
impl Atom for EmsgAtom {
    const FOUR_CC: FourCC = FourCC::new(b"emsg");
...
impl<R: Read + Seek> ReadAtom<&mut R> for EmsgAtom {
...
impl<W: Write> WriteAtom<&mut W> for EmsgAtom {
...
mod tests {
    ...
    fn test_emsg_version0() {
        ...
        assert_eq!(header.name, EmsgAtom::FOUR_CC);
        assert_eq!(src_box.size(), header.size);

        let dst_box = EmsgAtom::read_atom(&mut reader, header.size).unwrap();
