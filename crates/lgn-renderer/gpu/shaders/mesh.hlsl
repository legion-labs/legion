...
VertexOut main_vs(uint vertexId: SV_VertexID) {
    ...
    VertexIn vertex_in;
    if (HasPosition(mesh_info.format))
    {
        vertex_in.pos = static_buffer.Load<float4>(mesh_info.position_offset + vertexId * 16);
    }
    if (HasNormal(mesh_info.format))
    {
        vertex_in.normal = static_buffer.Load<float4>(mesh_info.normal_offset + vertexId * 16);
    }
    if (HasColor(mesh_info.format))
    {
        vertex_in.color = static_buffer.Load<float4>(mesh_info.color_offset + vertexId * 16);
    }
    if (HasTexCoord(mesh_info.format))
    {
        vertex_in.uv_coord = static_buffer.Load<float2>(mesh_info.tex_coord_offset + vertexId * 8);
    }
