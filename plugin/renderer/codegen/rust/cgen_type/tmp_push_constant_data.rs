// This is generated file. Do not edit manually

use std::mem;

use lgn_graphics_cgen_runtime::CGenTypeDef;

use lgn_graphics_cgen_runtime::prelude::*;

static TYPE_DEF: CGenTypeDef = CGenTypeDef {
    name: "TmpPushConstantData",
    id: 18,
    size: 16,
};

static_assertions::const_assert_eq!(mem::size_of::<TmpPushConstantData>(), 16);

#[derive(Clone, Copy)]
#[repr(C)]
pub struct TmpPushConstantData {
    data: [u8; 16],
}

#[allow(clippy::trivially_copy_pass_by_ref)]
impl TmpPushConstantData {
    pub const fn id() -> u32 {
        18
    }

    pub fn def() -> &'static CGenTypeDef {
        &TYPE_DEF
    }

    //
    // member : color
    // offset : 0
    // size : 16
    //
    pub fn set_color(&mut self, value: Float4) {
        self.set(0, value);
    }

    pub fn color(&self) -> Float4 {
        self.get(0)
    }

    #[allow(unsafe_code)]
    fn set<T: Copy>(&mut self, offset: usize, value: T) {
        unsafe {
            let p = self.data.as_mut_ptr();
            let p = p.add(offset as usize);
            let p = p.cast::<T>();
            p.write(value);
        }
    }

    #[allow(unsafe_code)]
    fn get<T: Copy>(&self, offset: usize) -> T {
        unsafe {
            let p = self.data.as_ptr();
            let p = p.add(offset as usize);
            let p = p.cast::<T>();
            *p
        }
    }
}

impl Default for TmpPushConstantData {
    fn default() -> Self {
        let mut ret = Self { data: [0; 16] };
        ret.set_color(Float4::default());
        ret
    }
}
