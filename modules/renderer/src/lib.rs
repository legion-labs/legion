    ...
    B: gfx_hal::Backend,
{
    pub fn new(
        ...
        let sparsely_bound = physical_device.features().contains(
        ...
        const STRIDE: usize = 4usize;
        let img = [0u8; WIDTH * HEIGHT * STRIDE];
        ...
        let image_upload_memory = unsafe {
            ...
            for y in 0..HEIGHT as usize {
                let row = &img[y * WIDTH * STRIDE..(y + 1) * WIDTH * STRIDE];
        ...
        if sparsely_bound {
            ...
            unsafe {
                queue_group.queues[0].bind_sparse(
                    ...
                    std::iter::empty::<(
                        &mut B::Buffer,
                        std::iter::Empty<&gfx_hal::memory::SparseBind<&B::Memory>>,
                    )>(),
                    std::iter::empty(),
                    std::iter::once((
                        &mut *image_logo,
                        std::iter::once(&gfx_hal::memory::SparseImageBind {
                            subresource: gfx_hal::image::Subresource {
                                aspects: gfx_hal::format::Aspects::COLOR,
                                level: 0,
                                layer: 0,
                            },
                            offset: gfx_hal::image::Offset::ZERO,
                            extent: gfx_hal::image::Extent {
                                width: WIDTH as i::Size,
                                height: HEIGHT as i::Size,
                                depth: 1,
                            },
