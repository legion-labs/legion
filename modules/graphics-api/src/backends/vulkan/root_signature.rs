use super::{VulkanApi, VulkanDeviceContext, VulkanSampler};
use crate::{
...
pub(crate) struct DescriptorInfo {
    ...
    //pub(crate) texture_dimensions: Option<TextureDimension>,
...
pub(crate) struct DescriptorSetLayoutInfo {
    ...
    pub(crate) descriptors: Vec<DescriptorIndex>,
    // Indexes binding index to the descriptors list
    pub(crate) binding_to_descriptor_index: FnvHashMap<u32, DescriptorIndex>,

    // --- vulkan-specific ---
    pub(crate) update_data_count_per_set: u32,
    // This indexes into the descriptors list
    pub(crate) dynamic_descriptor_indexes: Vec<DescriptorIndex>,
}
...
pub(crate) struct RootSignatureVulkanInner {
    ...
    pub(crate) name_to_descriptor_index: FnvHashMap<String, DescriptorIndex>,
    // Keeps them in scope so they don't drop
    immutable_samplers: Vec<VulkanSampler>, //empty_descriptor_sets: [vk::DescriptorSet; MAX_DESCRIPTOR_SETS],
...
impl VulkanRootSignature {
    ...
    pub fn find_descriptor_by_name(&self, name: &str) -> Option<DescriptorIndex> {
    ...
    pub(crate) fn descriptor(&self, descriptor_index: DescriptorIndex) -> Option<&DescriptorInfo> {
    ...
    pub fn vk_descriptor_set_layout(&self, set_index: u32) -> Option<vk::DescriptorSetLayout> {
    ...
    pub fn new(
        ...
        let vk_immutable_samplers: Vec<Vec<vk::Sampler>> = root_signature_def
            .immutable_samplers
            .iter()
            .map(|x| x.samplers.iter().map(|x| x.vk_sampler()).collect())
            .collect();
        ...
        let (pipeline_type, merged_resources, _merged_resources_name_index_map) =
            crate::backends::shared::merge_resources(root_signature_def)?;
        ...
        for resource in &merged_resources {
            ...
            if resource.resource_type != ResourceType::ROOT_CONSTANT {
                ...
                let immutable_sampler = crate::backends::shared::find_immutable_sampler_index(
                ...
                if immutable_sampler.is_some()
                    && !resource
                        .resource_type
                        .intersects(ResourceType::COMBINED_IMAGE_SAMPLER)
                {
                    ...
                    && vk_descriptor_type == vk::DescriptorType::COMBINED_IMAGE_SAMPLER
                {
                    ...
                    //
                    let descriptor_index = DescriptorIndex(descriptors.len() as u32);
