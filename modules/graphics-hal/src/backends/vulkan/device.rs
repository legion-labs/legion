...
use super::{command as cmd, conv, native as n, pool::RawCommandPool, window as w, Backend as B};
...
impl<'a> GraphicsPipelineInfoBuf<'a> {
    unsafe fn add_stage(&mut self, stage: vk::ShaderStageFlags, source: &pso::EntryPoint<'a, B>) {
        ...
        self.specializations.push(vk::SpecializationInfo {
            ...
            p_data: source.specialization.data.as_ptr().cast(),
        });
...
impl<'a> ComputePipelineInfoBuf<'a> {
    unsafe fn new(desc: &pso::ComputePipelineDesc<'a, B>) -> Self {
        ...
        this.specialization = vk::SpecializationInfo {
            ...
            p_data: desc.shader.specialization.data.as_ptr().cast(),
        };
        this
    }
}
...
impl d::Device<B> for super::Device {
        ...
        I: Iterator<Item = pso::Descriptor<'a, B>>,
    {
        ...
        for raw in raw_writes.iter_mut() {
            use super::vk::DescriptorType as Dt;
    ...
    unsafe fn copy_descriptor_set(&self, op: pso::DescriptorSetCopy<'_, B>) {
    ...
    unsafe fn set_display_power_state(
        &self,
        display: &crate::display::Display<B>,
        power_state: &crate::display::control::PowerState,
    ) -> Result<(), crate::display::control::DisplayControlError> {
        let display_control_extension = match self.shared.extension_fns.display_control {
            ...
            _ => return Err(crate::display::control::DisplayControlError::UnsupportedFeature),
        };

        let vk_power_state = match power_state {
            crate::display::control::PowerState::Off => vk::DisplayPowerStateEXT::OFF,
            crate::display::control::PowerState::Suspend => vk::DisplayPowerStateEXT::SUSPEND,
            crate::display::control::PowerState::On => vk::DisplayPowerStateEXT::ON,
        };
        ...
        match display_control_extension.display_power_control_ext(
            ...
            vk::Result::ERROR_OUT_OF_HOST_MEMORY => {
                Err(crate::display::control::DisplayControlError::OutOfHostMemory)
            }
            _ => unreachable!(),
        }
    }
    ...
    unsafe fn register_device_event(
        &self,
        device_event: &crate::display::control::DeviceEvent,
        fence: &mut <B as crate::Backend>::Fence,
    ) -> Result<(), crate::display::control::DisplayControlError> {
        let display_control_extension = match self.shared.extension_fns.display_control {
            Some(ref display_control_extension) => display_control_extension,
            _ => return Err(crate::display::control::DisplayControlError::UnsupportedFeature),
        ...
        let vk_device_event = match device_event {
            crate::display::control::DeviceEvent::DisplayHotplug => {
        ...
        match display_control_extension.register_device_event_ext(
            ...
            vk::Result::ERROR_FEATURE_NOT_PRESENT => {
                Err(crate::display::control::DisplayControlError::UnsupportedFeature)
    ...
    unsafe fn register_display_event(
        &self,
        display: &crate::display::Display<B>,
        display_event: &crate::display::control::DisplayEvent,
        fence: &mut <B as crate::Backend>::Fence,
    ) -> Result<(), crate::display::control::DisplayControlError> {
        let display_control_extension = match self.shared.extension_fns.display_control {
            Some(ref display_control_extension) => display_control_extension,
        ...
        let vk_display_event = match display_event {
            crate::display::control::DisplayEvent::FirstPixelOut => {
...
impl super::Device {
    ...
    pub(crate) unsafe fn create_swapchain(
    ...
    ) -> Result<(w::Swapchain, Vec<n::Image>), crate::window::SwapchainError> {
        ...
        let swapchain_raw = match result {
            ...
            Err(vk::Result::ERROR_SURFACE_LOST_KHR) => {
                return Err(crate::window::SurfaceLost.into())
            }
            Err(vk::Result::ERROR_NATIVE_WINDOW_IN_USE_KHR) => {
                return Err(crate::window::SwapchainError::WindowInUse)
            }
            Err(other) => {
                error!("Unexpected result - driver bug? {:?}", other);
                return Err(crate::window::SwapchainError::Unknown);
            }
        };
    ...
    /// Create image view from raw vulkan image
