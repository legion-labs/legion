...
/// Holds a vulkan application instance
...
/// Hold debug messages extension types
...
impl Version {
    /// Vulkan v1.0
    pub const V1_0: Self = Self(vk::make_version(1, 0, 0));
    /// Vulkan v1.1
    pub const V1_1: Self = Self(vk::make_version(1, 1, 0));
    /// Vulkan v1.2
    pub const V1_2: Self = Self(vk::make_version(1, 2, 0));
...
pub struct Instance {
    /// Contains the vulkan instance handle
...
unsafe extern "system" fn debug_utils_messenger_callback(
    ...
    let message_severity = match message_severity {
        ...
        _ => panic!("Unmapped DebugUtilsMessageSeverityFlagsEXT"),
    };
...
unsafe extern "system" fn debug_report_callback(
    ...
    let level = match type_ {
        ...
        _ => panic!("unmapped DebugReportFlagsEXT"),
    };
...
impl Instance {
    /// Lists the required extensions for a given version of vulkan
    pub fn required_extensions(
        entry: &Entry,
        driver_api_version: Version,
    ) -> Result<Vec<&'static CStr>, crate::UnsupportedBackend> {
        let instance_extensions = entry
            .enumerate_instance_extension_properties()
            .map_err(|e| {
                info!("Unable to enumerate instance extensions: {:?}", e);
                crate::UnsupportedBackend
            })?;
    ...
    /// Lists the required layers
    pub fn required_layers(entry: &Entry) -> Result<Vec<&'static CStr>, crate::UnsupportedBackend> {
        let instance_layers: Vec<vk::LayerProperties> =
        ...
        layers.retain(|&layer| {
            if instance_layers.iter().any(|inst_layer| unsafe {
    ...
    ) -> Result<Self, crate::UnsupportedBackend> {
        Self::inner_create(entry, raw_instance, true, driver_api_version, extensions)
    }
    ...
    fn inner_create(
            ...
            && !extensions.contains(&vk::KhrStorageBufferStorageClassFn::name())
        {
            ...
            return Err(crate::UnsupportedBackend);
        }
...
impl crate::Instance<Backend> for Instance {
    fn create(name: &str, version: u32) -> Result<Self, crate::UnsupportedBackend> {
        ...
        Self::inner_create(entry, instance, false, driver_api_version, extensions)
    }
    ...
    ) -> Result<window::Surface, crate::window::InitError> {
    ...
    unsafe fn create_display_plane_surface(
        &self,
        display_plane: &crate::display::DisplayPlane<'_, Backend>,
        plane_stack_index: u32,
        transformation: crate::display::SurfaceTransform,
        alpha: crate::display::DisplayPlaneAlpha,
        image_extent: crate::window::Extent2D,
    ) -> Result<window::Surface, crate::display::DisplayPlaneSurfaceError> {
        ...
        let surface_transform_flags = crate::display::SurfaceTransformFlags::from(transformation);
        ...
        let display_surface_ci = {
            ...
            match alpha {
                crate::display::DisplayPlaneAlpha::Opaque => {
                    builder.alpha_mode(vk::DisplayPlaneAlphaFlagsKHR::OPAQUE)
                }
                crate::display::DisplayPlaneAlpha::Global(value) => builder
                    .alpha_mode(vk::DisplayPlaneAlphaFlagsKHR::GLOBAL)
                    .global_alpha(value),
                crate::display::DisplayPlaneAlpha::PerPixel => {
                    builder.alpha_mode(vk::DisplayPlaneAlphaFlagsKHR::PER_PIXEL)
                }
                crate::display::DisplayPlaneAlpha::PerPixelPremultiplied => {
...
pub struct RawDevice {
    ...
    /// The `crate::Features::NDC_Y_UP` flag is implemented with either `VK_AMD_negative_viewport_height` or `VK_KHR_maintenance1`/1.1+. The AMD extension for negative viewport height does not require a Y shift.
    ///
...
impl RawDevice {
    ...
    fn map_viewport(&self, rect: &crate::pso::Viewport) -> vk::Viewport {
        let flip_y = self.features.contains(crate::Features::NDC_Y_UP);
...
impl queue::Queue<Backend> for Queue {
        ...
        Is: Iterator<Item = &'a native::Semaphore>,
    {
        ...
        let fence_raw = fence.map_or(vk::Fence::null(), |fence| fence.0);
...
/// Vulkand Backend implementation
pub struct Backend {}
