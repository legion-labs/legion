...
use crate::{format::Format, window as w};
...
impl Instance {
    ...
    /// Create a surface from a Khronos surface
            ...
            frames: images
                ...
                .map(|image| {
                    let view = device
                        .create_image_view(
                            image,
                            crate::image::ViewKind::D2,
                            format,
                            crate::format::Swizzle::NO,
                            usage,
                            crate::image::SubresourceRange {
                                aspects: crate::format::Aspects::COLOR,
                                ..Default::default()
                            },
                        )
                        .unwrap();
    ...
    unsafe fn acquire_image(
        ...
        match ssc.device.raw.wait_for_fences(fences, true, timeout_ns) {
            Ok(()) => {
                ...
                let image = Self::SwapchainImage {
                    ...
                    view: native::ImageView {
                        image: frame.image,
                        raw: frame.view,
                        range: crate::image::SubresourceRange {
                            aspects: crate::format::Aspects::COLOR,
                            ..Default::default()
                        },
                    },
            ...
            Err(vk::Result::ERROR_OUT_OF_HOST_MEMORY) => {
                Err(crate::device::OutOfMemory::Host.into())
            }
            Err(vk::Result::ERROR_OUT_OF_DEVICE_MEMORY) => {
                Err(crate::device::OutOfMemory::Device.into())
            }
            Err(vk::Result::ERROR_DEVICE_LOST) => Err(crate::device::DeviceLost.into()),
            _ => unreachable!(),
        }
    }
}
