...
impl PlatformMemory {
    #[cfg(any(unix, doc))]
    /// File descriptor from `PlatformMemory`
...
impl Into<PlatformMemoryType> for ExternalMemoryType {
    fn into(self) -> PlatformMemoryType {
        match self {
            ...
            #[cfg(windows)]
            ExternalMemoryType::OpaqueWin32
            | ExternalMemoryType::OpaqueWin32Kmt
            | ExternalMemoryType::D3D11Texture
            | ExternalMemoryType::D3D11TextureKmt
            | ExternalMemoryType::D3D12Heap
...
impl ExternalBufferMemory {
    ...
    pub fn platform_memory_type(&self) -> PlatformMemoryType {
        match self {
            ...
            #[cfg(windows)]
            Self::OpaqueWin32(_)
            | Self::OpaqueWin32Kmt(_)
            | Self::D3D11Texture(_)
            | Self::D3D11TextureKmt(_)
            | Self::D3D12Heap(_)
            ...
            Self::HostAllocation(_) | Self::HostMappedForeignMemory(_) => PlatformMemoryType::Ptr,
        }
    }
    ...
    pub fn handle(&self) -> Option<&Handle> {
        match self {
            Self::OpaqueWin32(handle)
            | Self::OpaqueWin32Kmt(handle)
            | Self::D3D11Texture(handle)
            | Self::D3D11TextureKmt(handle)
            | Self::D3D12Heap(handle)
    ...
    pub fn ptr(&self) -> Option<&Ptr> {
        match self {
            Self::HostAllocation(ptr) | Self::HostMappedForeignMemory(ptr) => Some(ptr),
