...
impl Offset {
    ...
    pub fn into_bounds(self, extent: &Extent) -> Range<Self> {
        let end = Self {
...
impl Usage {
    ...
    pub fn can_transfer(&self) -> bool {
        self.intersects(Self::TRANSFER_SRC | Self::TRANSFER_DST)
    }
...
impl Lod {
    /// Possible LOD range.
    pub const RANGE: Range<Self> = Self(f32::MIN)..Self(f32::MAX);
}
...
impl From<PackedColor> for [f32; 4] {
    fn from(val: PackedColor) -> Self {
        let mut out = [0.0; 4];
        for (i, channel) in out.iter_mut().enumerate() {
            let byte = (val.0 >> (i << 3)) & 0xFF;
