//! Low-level graphics abstraction for. Mostly operates on data, not types.
//! Designed for use by legion libraries and higher-level abstractions.
//!
...
//!
//! The first step to using `graphics-hal` is to initialize one of the available
...
/// Prelude module re-exports all the traits necessary to use `graphics-hal`.
pub mod prelude {
    pub use crate::{
...
bitflags! {
    ...
    pub struct Features: u128 {
        ...
        const DESCRIPTOR_INDEXING_MASK = Self::SAMPLED_TEXTURE_DESCRIPTOR_INDEXING.bits | Self::STORAGE_TEXTURE_DESCRIPTOR_INDEXING.bits | Self::UNSIZED_DESCRIPTOR_ARRAY.bits | Self::UNIFORM_BUFFER_DESCRIPTOR_INDEXING.bits | Self::STORAGE_BUFFER_DESCRIPTOR_INDEXING.bits;
        ...
        const MESH_SHADER_MASK = Self::TASK_SHADER.bits | Self::MESH_SHADER.bits;
