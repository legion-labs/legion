...
pub trait CommandBuffer<B: Backend>: fmt::Debug + Any + Send + Sync {
    ...
    unsafe fn begin_query(&mut self, query: query::Query<'_, B>, flags: query::ControlFlags);

    /// End a query.
    unsafe fn end_query(&mut self, query: query::Query<'_, B>);
    ...
    unsafe fn write_timestamp(&mut self, stage: pso::PipelineStage, query: query::Query<'_, B>);
