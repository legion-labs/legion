...
pub fn type_uuid_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let legion_util_path: Path = LegionManifest::default().get_path(crate::modules::LEGION_UTILS);
    ...
    for attribute in ast.attrs.iter().filter_map(|attr| attr.parse_meta().ok()) {
        ...
        if name_value.path.get_ident().map_or(true, |i| i != "uuid") {
            continue;
        }
    ...
    let gen = quote! {
        impl #legion_util_path::TypeUuid for #name {
            const TYPE_UUID: #legion_util_path::Uuid = #legion_util_path::Uuid::from_bytes([
                #( #bytes ),*
            ]);
        }
    };
