...
pub fn derive_render_resource(input: TokenStream) -> TokenStream {
    ...
    let bevy_render_path: Path = manifest.get_path(crate::modules::LEGION_RENDER);
    let bevy_asset_path: Path = manifest.get_path(crate::modules::LEGION_ASSET);
    let bevy_core_path: Path = manifest.get_path(crate::modules::LEGION_CORE);
